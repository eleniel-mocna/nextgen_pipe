...
{
    time {
        new/sort_sam.sh new/config_file.sh /data/Samuel_workdir/nextgen_pipe/CRISPR/1_marked_duplicates.sam | new/read_groups.sh | new/split_bam.sh | new/pileup.sh | new/call_variants.sh | new/merge_vcf.sh \
        | new/filter_vcf.sh \
        | tee \
            >(new/create_varfile.sh>/dev/null)\
            | new/snpSift.sh | new/snpEff.sh
